...
If the current branch has a PR, that PR is context only. The reviewed code is always the currently checked-out branch.
...
- `last 5 commits on main` is allowed only when `CURRENT_BRANCH=main`
...
- `Review the last 5 commits on main; only report issues introduced in those commits.`
- `Review the current branch feature/foo against origin/main; only report issues introduced by this branch.`
- `Review the current working tree on main; only report issues introduced by the current uncommitted changes.`

If `CURRENT_PR_NUMBER` exists, keep one short PR context string, for example:
...
2. filter to confidence `>= 75`
3. de-duplicate on `file + line + category + finding`
...
- start with the highest-severity, highest-confidence findings first
- group findings by severity: `critical`, `important`, `minor`
- for each finding, give a short bullet with `file:line`, the problem, and the impact
- if there are no findings, say that plainly
- mention worker failures clearly if any
- mention the review root path last, briefly
