...
Execute a feature spec from start to PR. This skill is the bridge between planning (`/living-spec` or `/spec`) and finalization (`/pr`).
...
If already on a feature branch, use it. If on `main`, create a new feature branch. The slug source depends on input mode:
...
> **NEVER push directly to main. NEVER.** This is non-negotiable.
...
2. **Create the PR by invoking the `/pr` skill.** Read the skill file at `~/.pi/agent/skills/pr/SKILL.md` and follow it.
   - **File mode:** `/pr` will auto-detect the spec from the branch name.
